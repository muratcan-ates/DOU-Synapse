...
> `cd apps/api && uv run pytest -q` → **882 passed** (9 Ağu, US1 sonrası). Sayı 002 boyunca artacak; tek doğrusu komutun kendisidir. <!-- docs-check: backend.tests = 882 -->
> `cd apps/web && bun test lib/` → **325 pass, 0 fail, 24 dosya.** <!-- docs-check: frontend.tests = 325 --><!-- docs-check: frontend.testFiles = 24 -->
...
```bash
cd apps/api
uv run pytest -q        # 882 passed, ~77 sn (9 Ağustos, US1 sonrası bu ağaçta ölçüldü)   # docs-check: backend.tests = 882
uv run mypy app
uv run ruff check . && uv run ruff format --check .

cd ../web
bun test lib/           # 325 pass, ~0,2 sn   # docs-check: frontend.tests = 325
bun run typecheck
```
