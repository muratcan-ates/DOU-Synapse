...
## D devam kabulü — 8 Eylül işletim görünürlüğü

- D1 dönem: bağımsız gerçek worker başlangıç temizliğinden sonra yeni eklenen expired satırı değiştirilmemiş 60 saniyelik sonraki turda siler; canlı satırın baytları aynı kalır. Bu yerel deney Docker veya scale-to-zero saklama SLA'sı değildir.
- OPS1: platform yetkisi ve mevcut admin SQL denetiminden sonra yönetim özeti, public readiness ile aynı DB/vector/kota/embedding yardımcı denetimini kullanır. Kota politika uyuşmazlığında hazırlık 503, yönetim durumu degraded olur; mevcut liveness ve yetki sınırları korunur. Admin aynı açık uygulama oturumunu kullanır. Kamusal uca admin toplamları eklenmez.
- OPS1 UI: iki ek bağımlılık durumu, eski API alan eksikliğinde Ölçülemedi; p95 örneklemi kaydedilmiş başarılı HTTP sohbet turları olarak açıklanır. Gerçek SQL uyuşmazlığı testi ile kontrollü UI yanıtı kanıtı ayrıdır. HTTP 200 kapsam reddi teknik başarılı örneğe dahildir; kalite puanı sayılmaz.
