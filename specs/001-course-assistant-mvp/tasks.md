...
- [ ] T043 [F] Evidence eşiği kalibrasyonu — `evaluation/evaluate.py --set calibration` ile `evidence_threshold` ayarlanır; seçilen değer + gerekçe YENİ: `evaluation/calibration_notes.md`'ye; holdout'a BAKILMAZ (Anayasa III). Kapsam dışı ret oranı sonra holdout'ta raporlanır. **DURUM (2026-08-09):** yöntem, kalibrasyon-holdout ayrımı kararı ve "holdout'a bakılmadı" beyanı `evaluation/calibration.md`'de. Eşik denemeleri KOŞULMADI — uçtan uca hat (Şerit 1+2) bekleniyor. `config.py`'deki KALİBRE EDİLMEMİŞTİR notu duruyor; değeri Şerit 1 güncelleyecek.
- [ ] T044 [F] Baseline (dense-only) vs hybrid (dense+FTS+RRF) koşusu — aynı holdout, eşleştirilmiş anlamlılık (bootstrap veya McNemar, en azından güven aralığı); "n=50 — yön göstergesi" kaydıyla sonuç `evaluation/results/`'a. **DURUM (2026-08-09):** KOŞULMADI. Eşleştirilmiş bootstrap (10.000 örnekleme, sabit tohum, %95 GA) ve McNemar'ın tam testi `evaluation/metrics.py`'de yazılı ve testli; T043 bitince koşulacak.
