...
**Doğrulama — 28 tablo görmelisiniz:** <!-- docs-check: tables.count = 28 -->
...
Depoda **20 migration dosyası** var (`0001`'den `0015`'e, aralıksız); hepsi tablo <!-- docs-check: migrations.count = 20 -->
...
```bash
cd apps/api
uv run pytest          # 1499 test yeşil olmalı (~50-100 sn)   # docs-check: backend.tests = 1499
...
(ölçüldü, 9 Ağustos: 3 istendi, 3 üretildi, şemadan geçti). Değişen tek şey cevabın
düzyazısını modelin yazmaması — bu yüzden "uç 200 döndü" ile "üretim çalışıyor" aynı
şey değildir.
