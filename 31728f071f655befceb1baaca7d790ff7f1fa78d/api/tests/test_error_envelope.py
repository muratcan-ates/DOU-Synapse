...
<<<<<<< HEAD
        # blueprint ailesi yedi, ingestion retry bir; kalite döngüsü iki path ekler.
        assert len(yollar) == 43, f"yol sayısı değişmiş: {len(yollar)}"
=======
        # blueprint ailesi yedi, ingestion yeniden denemesi bir, ürün portalı
        # (profil + dashboard + beş salt-okunur admin yolu) yedi path ekler.
        assert len(yollar) == 48, f"yol sayısı değişmiş: {len(yollar)}"
>>>>>>> f6b2ffc (Build the privacy-safe product portal backend)
