...
"exam_items_insert: RLS ve parent-lock guard draft sarti birlikte duserse|DROP TRIGGER exam_items_assessment_question_guard ON exam_items; DROP POLICY exam_items_instructor_insert ON exam_items; CREATE POLICY exam_items_instructor_insert ON exam_items FOR INSERT WITH CHECK (app.is_instructor(course_id));|exam_items_insert__yayinlanmis_kagida_soru_eklenemez"
