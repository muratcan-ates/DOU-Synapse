...
| C04 Documentation truth | `configured` | Docs owner / CODEOWNER | PR ve `main` push | `scripts/docs_check.mjs`; son local koşu retained evidence'da; GitHub run `unavailable` | Sayaç drift'i non-zero | Ruleset dışı bypass yok; bypass audit `unverified` | Required enforcement bilinmiyor / PR run URL'sini bağla | 2026-08-12 / Yok |
| C05 Real API/browser E2E | `configured` | QA owner / API+Web reviewer | Core E2E job | PostgreSQL + gerçek API + Playwright workflow'u; mevcut working-tree local koşusu yapılmadı | Setup, test veya teardown hatası job'u kırmızı yapar | Quarantine ancak issue, owner, coverage etkisi ve expiry ile | Candidate taşınabilirliği CI'da gözlenmedi / PR run, süre ve flake kaydını bağla | 2026-08-12 / Yok |
...
| C07 AI change governance | `configured` | AI owner / Course+Security reviewer | AI-sensitive reviewed diff | `.ai/` policy/schema ve local validator/mutasyon sonucu retained evidence'da; PR run/required enforcement `unavailable` | Eksik/hash uyuşmaz dossier ve geçersiz state fail closed | R2/R3 yazar bypass'ı yok; gerçek review/audit GitHub kaydı ister | Offline kapı production kalitesini kanıtlamaz / PR negative run ve ruleset bağla | 2026-08-12 / Yok |
