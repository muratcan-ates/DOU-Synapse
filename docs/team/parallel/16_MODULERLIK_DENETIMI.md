...
> Dal: `refactor/modularize`, taban `002-production-hardening` (2912056).
...
```
apps/api : 720 test yeşil · ruff check + format temiz
apps/web : 211 test yeşil · tsc --noEmit temiz
```

Suite süresi 224 sn → 36 sn. Test sayısındaki artış (697 → 720) eklenen
`test_text_tr.py`, `test_llm_json.py` ve kademe bekçisidir; web'deki düşüş
(216 → 211) silinen ikiz eşleme testleridir, iddia kaybı yok.
