...
- [x] T045 [P] [F] Embedding A/B — multilingual-e5-large vs bge-m3, ≥40 soru, Recall@5 + MRR; bge-m3 için geçici indeks (karar ingest-zamanıdır, üretim indeksi değişmez); sonuç "embedding seçim gerekçesi" başlığının verisi olur. **DONE (2026-08-09):** KOŞULDU, holdout'ta 105 puanlanabilir soru (istenen ≥40), iki ayrı veritabanı (`dou_synapse_eval` / `dou_synapse_eval_bge`), aynı materyal ve aynı üretim ingest hattı. **bge-m3'ün üstün olduğuna dair kanıt yok:** dense kolda İsabet@5 0,962→0,914 ve İsabet@8 0,981→0,933 (GA [−0,095,−0,010], McNemar p=0,0625, 5 ayrışan soru), MRR farkı sıfırdan ayrılmıyor; hibrit kolda dört ölçütün dördü de ayrılmıyor. bge-m3 fastembed dense kataloğunda olmadığı için resmî ONNX dışa aktarımından onnxruntime ile koşturuldu (`evaluation/embedding_bge_m3.py`); üretim paketine girmedi, `config.py` değişmedi. Üretim indeksi DEĞİŞMİYOR. Tam analiz `evaluation/embedding_ab.md`.
- [x] T046 [F] Injection + Sokratik sızıntı koşusu — ≥15 injection vakası ve sızıntı senaryoları guardrail zincirinden geçirilir; sızıntı/ihlal oranı kaydedilir; rapor dili "bilinen temel kalıplara karşı smoke-test edildi" düzeyinde tutulur ("dayanıklı" DENMEZ). **DONE (2026-08-09), KISMEN:** `evaluation/injection/cases.json` — 38 vaka, altı kategori (istenen ≥15); holdout'taki 21 injection/sızıntı kaydının tamamı bağlandı (34 bağ, `link_holdout.py --check` iki yönlü denetliyor). Koşu gerçek anahtar olmadan yapıldığı için denetimler İKİ SINIFA ayrıldı: **deterministik 3/35 ihlal (GEÇERLİ)** — sınav modu 2/2 HTTP 422, Sokratik kademe 13/13 ilerlemedi, atıf uydurtma 3/3 temiz; **LLM'e bağlı 17 vaka KOŞULMADI** — sahte sağlayıcı çözüm üretmediği için sızıntı oranı 0/12 triviyal, rapora girmiyor. **AÇIK KUSUR:** kapsam dışına kayan sorular kanıt kapısını geçiyor (INJ-030/031/032 best_dense 0,826/0,841/0,836 > 0,81); ders sözcük dağarcığıyla başlayan soru eşiği aşıyor. Düzeltme R4/Şerit 1'in. `review.md` üretildi, BOŞ — insan incelemesi atlanamaz. Ayrıntı `evaluation/injection/README.md`.
- [ ] T047 [F] Faithfulness örneklemi — 20-30 gerçek cevap çekilir, YENİ: `evaluation/faithfulness/sample_template.md` etiketleme şablonu; 2 kişi bağımsız etiketler, uyum oranı kaydedilir. **DURUM (2026-08-09):** ETİKETLEME KOŞULMADI — gerçek LLM anahtarı yok (GROQ/GEMINI/EVAL üçü de boş). Yapılanlar: `pull_sample.py` yazıldı; 25 cevap sabit tohumla (20260809) gerçek API'den, gerçek guardrail zincirinden geçirilerek çekildi (`sample_2026-08-09.json`); iki etiketleyici için ayrı dosya üretildi, ikisi de BOŞ ve kaynak parçaların metnini taşıyor. Örneklem sahte sağlayıcıyla çekildiği için GEÇERSİZ: cevap zaten kaynağın özeti, "kaynağa sadık" etiketi totoloji olurdu. **"İki kişi etiketledi" YAZILMADI** — bu şeridi tek ajan koşturdu. Anahtar gelince aynı tohumla aynı 25 soru çekilir.
