...
import { recordE2eApiResponses } from "./fixtures";
...
async function sor(page: Page, soru: string) {
  ...
  await expect(page.getByText("Gönderiliyor…")).toHaveCount(0, {
    timeout: 180_000,
  });
}
...
test.describe("belge ekran görüntüleri @ekran", () => {
  ...
  test("sohbet — kaynaklı cevap ve önbellek işareti", async ({ page }) => {
    ...
    await expect(
      page.getByText("Sayfa", { exact: false }).first(),
    ).toBeVisible();
  ...
  test("sohbet — kapsam dışı soruda nazik ret", async ({ page }) => {
    ...
    await expect(page.getByRole("main").locator('[role="alert"]')).toHaveCount(
      0,
    );
    await cek(page, "10-sohbet-nazik-ret");
  });

  test("soru havuzu — üretim raporu ve elenme gerekçeleri", async ({
    page,
  }) => {
    await girisYap(page, AYSE);
    await page.goto(`/courses/${DERS}/questions`);
    await expect(
      page.getByRole("heading", { name: "Soru havuzu" }),
    ).toBeVisible();
  ...
  test("ilerleme — sınıf analitiği", async ({ page }) => {
    ...
    await expect(
      page.getByText("resmî bir not değildir", { exact: false }),
    ).toBeVisible();
  ...
  test("KVKK aydınlatma metni", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "KVKK Aydınlatma Metni" }),
    ).toBeVisible();
