...
let disabledApi: string;

function requireDisabledApi(): string {
  const value = process.env.E2E_DISABLED_API_URL;
  if (!value) {
    throw new Error(
...
test.describe("release readiness fail-closed kapıları", () => {
  ...
  test.beforeAll(() => {
    // `--list` belge sayacını canlı servis istemeden çalıştırabilsin; gerçek
    // koşu ise ikinci API yoksa daha ilk hook'ta kırmızı olur, test atlanmaz.
    disabledApi = requireDisabledApi();
  });
  ...
  test("global kill switch UI ve ham POST'u aynı anda kapatır", async ({ page }) => {
    ...
    }>(disabledApi, disabledPath, { userId: BURAK.id });
    ...
    await page.route(`${API}${disabledPath}`, async (route) => {
      const response = await page.request.get(`${disabledApi}${disabledPath}`, {
        headers: {
