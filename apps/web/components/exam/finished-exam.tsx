...
export function FinishedExam({ courseId, session, finish, questions, onRestart, historyEnabled }: {
  courseId: string;
  session: ExamSession;
  ...
  const loadResult = useCallback(async (): Promise<ResultView> => {
    ...
    try {
      ...
      return { result: result.results_locked ? null : result, lockedMessage: result.results_locked ? result.message : null };
    } catch (error) {
      ...
      if (error instanceof ApiError && error.code === "exam_in_progress") {
        return { result: null, lockedMessage: error.message };
      }
      throw error;
    }
  ...
  if (result.error) return <div className="mx-auto max-w-2xl">{title}<ErrorNote message={result.error} onRetry={result.reload} /></div>;
  ...
  const score = formatScore(completed ? completed.score : session.score);
  const answered = completed ? completed.answered_count : session.answered_count;
  const unanswered = completed ? completed.unanswered_count : session.question_count - session.answered_count;
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {result.refreshError && <div className="mb-4"><ErrorNote message={result.refreshError} onRetry={result.reload} /></div>}
      <MetricRow items={[
