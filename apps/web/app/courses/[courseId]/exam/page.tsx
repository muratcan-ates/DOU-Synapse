  ...
  return (
    <AppShell>
      ...
      {!identity.ready || !identity.user ? <Loading /> : (
        <ExamScreen key={`${courseId}:${identity.user.id}`} courseId={courseId} userId={identity.user.id} />
      )}
    </AppShell>
  );
}
...
function ExamScreen({ courseId, userId }: { courseId: string; userId: string }) {
  ...
  }, [courseId, sessionId, restored, storageKey]);
  ...
  return (
    <RunningExam key={session.id} courseId={courseId} session={session}
