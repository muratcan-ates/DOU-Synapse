...
- [x] T114 [US1] Kilidin ek SELECT maliyeti: yeni indeks **eklenmez** — `exam_sessions_user_idx` (`0004_assessment.sql:94`) sorgunun öncü yüklemini ve sıralamasını karşılıyor. Ölçüm teslim öncesi koşulur ve `docs/test-report.md`'ye yazılır: `EXPLAIN (ANALYZE, BUFFERS)` planı + p50/p95. Eşik aşılırsa doğru düzeltme kısmi indekstir (`… WHERE finished_at IS NULL`), ama ölçüm göstermeden eklenmez. **DONE (2026-08-10):** PostgreSQL 16'da yaklaşık 200 bin oturumlu gerçekçi dağılımda mevcut indeks kullanıldı; execution 0,052 ms, 1000 tekrarda kilit SELECT'i p95 0,058–0,061 ms ve tabana ek p95 0,036–0,039 ms. Kısmi indeks eklenmedi. `request_logs` kronometresinin bağımlılıklardan sonra başladığı ve kilit SELECT'ini kapsamadığı fark edildi; uydurma uç farkı yerine aynı RLS/GUC bağlamında sorgu doğrudan ölçüldü. Patolojik tek-kullanıcı/100 bin oturum sınırı ayrıca p95 5,016–5,030 ms olarak raporlandı (`docs/test-report.md` §14b).
...
**DONE (2026-08-10):** T202–T207 tamamlandı; son madde gerçek yerel ONNX runtime
ile ölçüldü, sahte sağlayıcı yalnız LLM cevap kalitesi sınırı olarak açıkça ayrıldı.
- [x] T207 [US2] `docs/test-report.md` — bloke süresi ölçümü (SC-014) ve ısıtma sonrası ilk istek süresi. **DONE (2026-08-10):** gerçek 2,1 GB `multilingual-e5-large` ile 60 sayfalık belge işlenirken 1899/1899 sağlık yoklaması 200; p95 4,1 ms, max 150,2 ms. Arka plan ısıtma 15,184 sn; ısıtma sonrası ilk soru 123,2 ms. Warmup kapalı süreç-soğuk karşı kontrol 1603,4 ms, ikinci sıcak soru 59,9 ms. Koşullar ve sınırlılık §14b'de.
...
- [x] T305 [P] [US8] `docs/team/parallel/20_DEVIR_9_AGUSTOS.md` §4a — "soru üretimi sıfır soru döndürüyor" bayat (67ee442'de düzeltilmiş, `test_uretilen_taslak_havuza_kadar_gider` yeşil). Belge tarihsel kayıt olduğu için **silinmez**, düzeltme notu düşülür.
- [x] T306 [US8] Eşik metriğinin tek doğrusu: `%80` (ARCHITECTURE §10 md.7 + README 226-231) `calibration.md` §7'nin **süperseded** v1 ölçümünden (n=10); §8 v2'de 0,81'de 11/18 = %61; `test-report` §3'te %50 (11/22). Üç farklı payda aynı metriğe atanmış. Hangi setin hangi sayıyı verdiği **tek yerde** netleştirilir; `scope.py` indikten sonra yeniden ölçüm yapılmadığı için nihai sayı KOŞULMADI kalır.
- [x] T307 [US8] YENİ: `scripts/docs_check.mjs` — genel amaçlı doğal dil doğrulayıcı **yazılmaz** (gerçekçi değil). Yerine üç dar mekanik kapı: (a) belgelerdeki test sayısı ile `pytest --collect-only -q` çıktısı, (b) tablo sayısı ile `information_schema` sayımı, (c) migration listesi ile `ls supabase/migrations`. Bağımlılıksız Node ESM, `apps/web/scripts/contrast.mjs` deseninde. `.github/workflows/ci.yml`'e adım olarak eklenir. Bu, 664/668 gibi kaymaların bir daha elle yakalanmasını gerektirmez.

**DONE (2026-08-10):** Belgeler kodla yeniden hizalandı; tarihsel sayılar tarih
damgasıyla korundu, canlı sayılar mekanik `docs_check` kapısına bağlandı.
...
**DONE (2026-08-10):** Tam blueprint, kaynak sürümü, yayın penceresi, değişmez sınav
sürümü, rubrik kırılımı ve eğitmen ekranı tamamlandı.
...
- [x] T602 [US4] YENİ: `apps/api/app/modules/policy/service.py` — tek çözümleyici `resolve_policy(session, *, course_id, settings)`. Döndürdüğü frozen dataclass'ta hiçbir alan Optional değildir; her değer çözülmüş gelir. FR-136 kapısı: politikası olmayan ders **bugünkü davranışla** çalışır ve bu birim testle sabitlenir.
...
- [x] T605 [US4] `apps/api/app/modules/assessment/socratic.py` + `apps/api/app/core/config.py` — ipucu üst sınırı bugün **iki ayrı yerde** sabit ve birbirinden habersiz (`socratic.py:68-79` beş kademe, `config.py:221` dört); tek sözlüğe indirilir (Anayasa XI).
...
- [x] T609 [US4] `apps/api/tests/test_policy.py` (YENİ) — FR-136 regresyonu: politika inmeden önce kaydedilen cevap, indikten sonra **aynı** olmalı.

**DONE (2026-08-10):** Ders politikası sunucu tarafında zorlanıyor; günlük bütçenin
kapsamı açıkça "sohbet token'ları", soru üretimi ise ayrı maliyet kotasıdır.

---

## Blok 7 — Sayfalama (US6) · FR-160…FR-163

- [x] T701 [US6] YENİ: `apps/api/app/schemas/page.py` — generic `Page[T] { items, next_cursor, total }`; `apps/api/app/api/deps.py`'ye `PageDep` (`limit` sunucu tarafında `min(limit, 100)` ile **kırpılır**, 422 değil).
- [x] T702 [US6] YENİ: `supabase/migrations/0011_pagination_indexes.sql` — belirlenimci sıralama indeksleri. Veri birikmiş bir üretim veritabanında `CONCURRENTLY` ve işlem dışında; eski indeksler yenileri kurulduktan **sonra** silinir.
...
**DONE (2026-08-10):** Beş liste keyset cursor'a geçti. T701 taslağındaki `total`
bilinçli çıkarıldı: `next_cursor` devam bilgisini taşır; her sayfada COUNT çalıştırmak
SC-008'in sabit ilk-sayfa maliyetiyle çelişir (`data-model.md` §8 karar 6).

---

## Blok 8 — Gerçek kimlik (US7) · FR-170…FR-173

- [x] T801 [US7] `apps/web/package.json` + YENİ: `apps/web/lib/supabase.ts` — `@supabase/supabase-js` eklenir (anahtarlar sonra gelir). `PLAN.md` "Teknoloji Kilidi" bölümüne yazılı gerekçe düşülür. Anahtar YOKKEN de derlenir ve `dev:` kimliği çalışmaya devam eder.
...
- [x] T901 [P] [US9] `apps/web/e2e/` — `globalTeardown` + test verisi deseni; koşu sonunda bıraktığı kalıcı kayıt **sıfır** (SC-010). Test ortada düşse de temizlik çalışır.
- [x] T902 [P] [US9] YENİ: temizlik komutu — ne sileceğini **önce gösterir**, sonra onay ister.
...
- [x] T906 [P] [US11] YENİ: `apps/web/lib/security-headers.ts` + `apps/web/next.config.ts` + `apps/api/app/main.py` — **tek sözlük, iki uygulama noktası**: Next HTML/JS gönderiyor, FastAPI JSON; aynı başlık kümesi ikisine de anlamlı değil.

**DONE (2026-08-10):** E2E teardown ve güvenli temizlik, KVKK hakları, ingestion
retry/backoff, güvenlik başlıkları tamamlandı. Ek olarak production Supabase Storage
adaptörü ve Compose'ta API/worker ortak belge volume'u uygulandı.
